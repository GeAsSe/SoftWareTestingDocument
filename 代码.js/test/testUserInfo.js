
/**
 * 【特别提醒】cookie 是用户的敏感信息，此处只能是**测试**用户的 cookie
 * 每次测试用户重新登录，都需要更新这里的 cookie
 */

// module.exports = {
//     Z_ID: 1,
//     Z_USER_NAME: 'zhangsan',
//     Z_COOKIE: 'weibo.sid=8YVHHiofAiPqNvHHt91Rw7SZaXOfivab; weibo.sid.sig=q_gnrP-vM1ywHaNpZSlHst1XmgQ',

//     L_ID: 2,
//     L_USER_NAME: 'lisi',
//     L_COOKIE: 'weibo.sid=686m0u92zMTCmmSQY85trYv_PScDgpFx; weibo.sid.sig=Za_Oj-wSLFrG5QzQhqQU0YQxOFo'
// }

module.exports = {
    Z_ID: 5,
    Z_USER_NAME: 'zx',
    Z_COOKIE: 'weibo.sid=xGCO3gPaL9s3n7uQ7oOnGfKJ_ONcfwLg; weibo.sid.sig=SKLliJVBWMvTfsy8juywoEonOoQ',

    L_ID: 7,
    L_USER_NAME: 'OnlyEnglish',
    L_COOKIE: 'weibo.sid=jcpB1JOZnHGx0Cpuc4llYnUxYOXe9Xgz; weibo.sid.sig=oF-hPWwFiDggwdQnpoIfrx3NnGM'
}
